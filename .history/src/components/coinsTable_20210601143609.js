...
export default function CoinsTable() {
  const renderCoinsTable = () => {
    ...
    let view = `<div class="${styles.coins}"> ${SelectFiat()} <div class="${styles.coinsTable}"> ${generateCoinsTableHeader()} <div class="${styles.tbody}">
      </div>`;
    const {
      coinsDataDisplay,
      coinsDataUpd,
      ...
      view += `<div class="
                ...
                ${
                  elem.filter === window.dataStorage.activeFilter['attr']
                    ...
                    : ''
                }" onclick="${Filters(this)}window.filter(this)" data-filter="${elem.filter}">
