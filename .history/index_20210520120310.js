...
window.rearengeData = coinsDataRaw => {
  ...
  for (let coin in coinsDataRaw) {
    for (let fiat in coinsDataRaw[coin]) {
      coinsDataRaw[coin][fiat]['coinName'] = window.dataStorage.coinsDataFull[coin][fiat]['FROMSYMBOL'];
      coinsDataRaw[coin][fiat]['coinPrice'] = window.dataStorage.coinsDataFull[coin][fiat]['PRICE'];
      coinsDataRaw[coin][fiat]['coinChange'] = window.dataStorage.coinsDataFull[coin][fiat]['CHANGEPCTDAY'];
      coinsDataRaw[coin][fiat]['coinCap'] = window.dataStorage.coinsDataFull[coin][fiat]['MKTCAP'];
      coinsDataRaw[coin][fiat]['coinVolume'] = window.dataStorage.coinsDataFull[coin][fiat]['VOLUMEDAYTO'];
    }
    coinsDataUpd.push(coinsDataRaw[coin]);
  }

  console.log(coinsDataRaw);
